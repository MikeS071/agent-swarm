...
var promptsValidateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, _ []string) error {
		...
		for _, ticketID := range ticketIDs {
			result := promptTicketResult{Ticket: ticketID, Valid: true}
			...
			if readErr != nil {
				if os.IsNotExist(readErr) {
					result.Valid = false
					failure := promptTicketFailure{Rule: promptRuleMissingFile, Message: "prompt file not found"}
					result.Failures = append(result.Failures, failure)
					flatFailures = append(flatFailures, promptValidationFailure{Ticket: ticketID, Rule: failure.Rule, Message: failure.Message})
				} else {
				...
				for _, failure := range report.Failures {
					ticketFailure := promptTicketFailure{Rule: failure.Rule, Message: failure.Message}
