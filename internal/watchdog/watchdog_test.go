...
func TestSpawnTicketGuardianChecks(t *testing.T) {
	tests := []struct {
		...
		{
			name: "happy path enforce mode valid flow allows spawn",
			flowContent: `
version: 1
...
`,
		...
		{
			name: "error path enforce mode invalid flow blocks spawn",
			flowContent: `
version: 1
name: invalid-flow
modes:
...
`,
		...
		{
			name: "edge path advisory mode invalid flow warns but spawns",
			flowContent: `
version: 1
name: invalid-flow
modes:
...
`,
		...
		{
			name:               "edge path missing flow file skips guardian and spawns",
			writeFlow:          false,
			wantSpawnCalls:     1,
			wantStatus:         tracker.StatusRunning,
			wantGuardianResult: "",
